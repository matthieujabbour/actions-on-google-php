<?php
  ...
  require_once(__DIR__ . '/assistant-app.php');
   ...
   */
  final class ActionsSdkApp extends AssistantApp {
    /**
    ...
    public function __construct ($options) {
      ...
      $requestData = function() { return $this->body_; };
      parent::__construct($options, $requestData->bindTo($this, $this));
      ...
      if ($this->body_ &&
          isset($this->body_['conversation']) &&
          isset($this->body_['conversation']['type']) &&
          $this->body_['conversation']['type'] === $this->ConversationStages['NEW'] &&
          $this->sessionStarted_ && is_callable($this->sessionStarted_)) {
        $this->sessionStarted_();
      } else if ($this->sessionStarted_ && !is_callable($this->sessionStarted_)) {
  ...
  //   }
    ...
    public function getRawInput () {
      debug('getRawInput');
      $input = $this->getTopInput_();
      if (is_null($input)) {
        error('Failed to get top Input.');
        return null;
      }
      if (!isset($input['rawInputs']) || isset($input['rawInputs']) && count($input['rawInputs']) === 0) {
        error('Missing user raw input');
        return null;
      }
      $rawInput = $input['rawInputs'][0];
      if (!isset($rawInput['query'])) {
  ...
  //   }
     ...
     * @actionssdk
     */
    public function ask ($inputPrompt, $dialogState = []) {
      debug('ask: inputPrompt=%s, dialogState=%s',
        json_encode($inputPrompt), json_encode($dialogState));
      $expectedIntent = $this->buildExpectedIntent_($this->StandardIntents['TEXT'], []);
      if (is_null($expectedIntent)) {
  ...
  //   }
     ...
     * @actionssdk
     */
    public function tell ($textToSpeech) {
      debug('tell: textToSpeech=%s', $textToSpeech);
      if (is_null($textToSpeech)) {
        $this->handleError_('Invalid speech response');
        return null;
      }
      $finalResponse = [];
      if (is_string($textToSpeech)) {
        ...
        if (isset($textToSpeech['items'])) {
          $finalResponse['richResponse'] = $textToSpeech;
        } else if (isset($textToSpeech['speech'])) {
      ...
      $response = $this->buildResponseHelper_(null, false, null, $finalResponse);
     ...
     * @actionssdk
     */
    public function buildInputPrompt ($isSsml, $initialPrompt, $noInputs = []) {
      debug('buildInputPrompt: isSsml=%s, initialPrompt=%s, noInputs=%s',
        $isSsml, $initialPrompt, $noInputs);
      $initials = [];

      if ($noInputs) {
        if (count($noInputs) > INPUTS_MAX) {
  ...
  // // ---------------------------------------------------------------------------
     ...
     * @actionssdk
     */
    private function getTopInput_ () {
      debug('getTopInput_');
      if (!isset($this->body_['inputs']) || isset($this->body_['inputs']) && count($this->body_['inputs']) === 0) {
     ...
     * @actionssdk
     */
    private function buildResponseHelper_ ($conversationToken, $expectUserResponse, $expectedInput, $finalResponse) {
      debug('buildResponseHelper_: conversationToken=%s, expectUserResponse=%s, ' .
      'expectedInput=%s, finalResponse=%s',
      $conversationToken, $expectUserResponse, json_encode($expectedInput),
      json_encode($finalResponse));
      $response = [];
      if (!is_null($conversationToken)) {
        $response['conversationToken'] = $conversationToken;
      }
      $response['expectUserResponse'] = $expectUserResponse;
      if (!is_null($expectedInput)) {
        $response['expectedInputs'] = $expectedInput;
      }
      if (!$expectUserResponse && !is_null($finalResponse)) {
     ...
     * @actionssdk
     */
    private function maybeAddItemToArray_ ($item, &$array) {
  ...
  //   }
    ...
    private function buildAskHelper_ ($inputPrompt, $possibleIntents, $dialogState) {
      ...
      $response = $this->buildResponseHelper_(
        json_encode($dialogState),
    ...
    * @actionssdk
    */
    private function buildExpectedIntent_ ($intent) {
      debug('buildExpectedIntent_: intent=%s', $intent);
      if (is_null($intent) || $intent === '') {
