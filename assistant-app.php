<?php
  ...
  class AssistantApp {
    ...
    public $data;
    public $ConversationStages;
    public $StandardIntents;
    protected $request_;
    protected $response_;
    protected $responded_;
    protected $body_;
    protected $handler_;
    protected $actionsApiVersion_;
    protected $sessionStarted_;
     ...
     */
    public function __construct ($options, $requestData) {
       ...
       * @type {Function}
       */
      $this->sessionStarted_ = isset($options['sessionStarted']) ? $options['sessionStarted'] : null;

      debug('Request from Assistant: %s', json_encode($this->request_->body()));
       ...
       * @type {Object}
       */
      $this->body_ = json_decode(json_encode($this->request_->body()), true);
      ...
      if (isset($this->body_['originalRequest']) &&
      isset($this->body_['originalRequest']['version'])) {
    ...
    public function handleRequest ($handler) {
      ...
      if (is_callable($handler)) {
        ...
        call_user_func($this->handler_, $this);
        return;
      } else if (is_array($handler)) {
        debug('handleRequest: map');
        $intent = $this->getIntent();
        $result = $this->invokeIntentHandler_($handler, $intent);
        if (!$result) {
      ...
      $this->handleError_('invalid intent handler type: ' . gettype($handler));
  ...
  //   }
     ...
     * @private
     */
    protected function isSsml_ ($text) {
      debug('isSsml_: text=%s', $text);
      if (empty($text)) {
        error('Text can NOT be empty');
        return false;
      }
      return Responses\isSsml($text);
    }
     ...
     * @private
     */
    protected function isNotApiVersionOne_ () {
      debug('isNotApiVersionOne_');
      return !is_null($this->actionsApiVersion_) &&
        intval($this->actionsApiVersion_) >= ACTIONS_CONVERSATION_API_VERSION_TWO;
    }
     ...
     * @private
     */
    protected function handleError_ ($text) {
      ...
      // Log error
      error($text, array_slice(func_get_args(), 1));
     ...
     * @private
     */
    protected function doResponse_ ($response, $responseCode) {
      debug('doResponse_: response=%s, responseCode=%d', json_encode($response), $responseCode);
      if ($this->responded_) {
        return;
      }
      if (is_null($response)) {
      ...
      debug('Response %s', json_encode($response));
      $httpResponse = $this->response_->status($code)->send($response);
      $this->responded_ = true;
      return $httpResponse;
    }
     ...
     * @private
     */
    protected function extractData_ () {
      debug('extractData_');
      $this->data = [];
    }
     ...
     * @private
     */
    protected function buildPromptsFromSsmlHelper_ ($ssmls) {
      debug('buildPromptsFromSsmlHelper_: ssmls=%s', $ssmls);
      $prompts = [];
      for ($i = 0; $i < count($ssmls); $i++) {
        $prompt = [
          'ssml' => $ssmls[$i]
        ];
     ...
     * @private
     */
    protected function buildPromptsFromPlainTextHelper_ ($plainTexts) {
